    ...
    for category, trade_list in [("Ganadores", winning_trades), ("Perdedores", losing_trades), ("Todos", trades)]:
        ...
        for trade in trade_list:
            if not hasattr(trade, 'day_candles'):
                ...
                trade.returns = pd.DataFrame([c.to_dict() for c in trade.day_candles])['close'].pct_change().cumsum().dropna()
...
def calculate_statistics(trades: list[Trade]) -> dict:
    ...
    daily_returns_list = []  # Lista para almacenar DataFrames de retornos diarios
    
    # Para el heatmap 2D: hora + precio
    min_hour_price = []  # [(hora, precio_relativo)]
    max_hour_price = []  # [(hora, precio_relativo)]
    
    processed_symbols = set()  # Para rastrear símbolos ya procesados
    ...
    for trade_idx, trade in enumerate(trades):
        ...
        regular['returns'] = regular['close'] / day_open - 1
        
        # Crear DataFrame para retornos diarios con nombre único de columna
        # Usar símbolo del trade si está disponible, sino usar índice
        column_name = getattr(trade, 'symbol', f'Trade_{trade_idx}')
        
        # Solo procesar si no hemos visto este símbolo antes
        if column_name not in processed_symbols:
            processed_symbols.add(column_name)
            daily_df = regular[['date', 'returns']].copy()
            daily_df['time'] = daily_df['date'].dt.time
            daily_df = daily_df[['time', 'returns']].rename(columns={'returns': column_name})
        ...
        min_return = ((min_price - day_open) / day_open)
        max_return = ((max_price - day_open) / day_open)
        ...
        # Retornos por hora
        regular['hour'] = regular['date'].dt.hour
        regular['returns_pct'] = regular['close'].pct_change()
    ...
    # Concatenar todos los DataFrames de retornos diarios
    if daily_returns_list:
        # Usar outer join para incluir todos los horarios
        daily_returns_combined = pd.concat(daily_returns_list, axis=1, join='outer', sort=True)
        # Aplicar forward fill para rellenar valores faltantes
        daily_returns_combined = daily_returns_combined.fillna(method='ffill')
    else:
    ...
    return {
        ...
        'daily_ret': daily_returns_combined
    }
...
def create_heatmap_visualization(results:dict[str, dict]):
    ...
    fig.show()
    return fig
def create_probabilistic_scatter_fixed_old(results):
    """
...
def create_probabilistic_scatter_fixed(results):
    ...
    min_data = defaultdict(list)  # {hora: [retornos]}
    max_data = defaultdict(list)  # {hora: [retornos]}
    cumulative_series = defaultdict(list)

    total_days = max([stats['total_days'] for stats in results.values()] + [1])
    results['Todos']['daily_ret'] = results['Todos']['daily_ret'].sort_index()

    # Recorremos cada trade/category para encontrar mínimos y máximos
    for col_idx in range(len(results['Todos']['daily_ret'].columns)):
        c = results['Todos']['daily_ret'].iloc[:, col_idx]
        
        # Encontrar índices de mínimo y máximo
        min_idx = c.idxmin()
        max_idx = c.idxmax()  # Corregido: era idxmin()
        
        # Obtener valores de retorno en esos puntos
        min_return = c.loc[min_idx] * 100  # Convertir a porcentaje
        max_return = c.loc[max_idx] * 100  # Convertir a porcentaje
        
        # Agrupar por hora
        min_hour = min_idx.hour
        max_hour = max_idx.hour
        
        # Agregar a las listas por hora
        min_data[datetime(1970, 1, 1, hour=min_hour, minute=30).time()].append(min_return)
        max_data[datetime(1970, 1, 1, hour=max_hour, minute=30).time()].append(max_return)

    # Procesar datos de retornos por hora
    for stats in results.values():
        for h, returns in stats['hourly_returns'].items():
            cumulative_series[h].extend([ret*100 for ret in returns])

    # Función para preparar scatter con agrupación por hora
    def prepare_scatter_grouped(data_by_hour, color, name, total_days):
        xs, ys, sizes = [], [], []
        
        for hour, returns_list in data_by_hour.items():
            if returns_list:  # Si hay datos para esta hora
                # Calcular retorno medio para esta hora
                mean_return = np.mean(returns_list)
                
                # Probabilidad = número de ocurrencias / total de días
                probability = len(returns_list) / total_days
                
                # Solo agregar si la probabilidad es mayor que 0
                if probability > 0:
                    xs.append(hour)
                    ys.append(round(mean_return, 1))  # Redondear a 1 decimal
                    sizes.append(probability * 100)  # Escalar tamaño visual (aumenté el factor)
        
        return go.Scatter(
            x=xs, y=ys, mode='markers',
            marker=dict(size=sizes, color=color, opacity=0.6),
            name=name,
            hovertemplate=f'{name}<br>Hora: %{{x.hour}}:00<br>Retorno Medio: %{{y:.1f}}%<br>Probabilidad: %{{customdata:.1f}}%<extra></extra>',
            customdata=[s for s in sizes]  # Para mostrar probabilidad en hover
        )
    ...
    # Curva de retorno acumulado medio
    fig.add_trace(go.Scatter(
        x=results['Todos']['daily_ret'].index,
        y=results['Todos']['daily_ret'].mean(axis=1) * 100,  # Convertir a porcentaje
        mode='lines+markers',
        line=dict(color='blue', width=3),
        marker=dict(size=6, symbol='diamond'),
        name='Retorno Acumulado Medio',
        hovertemplate='Hora: %{x}<br>Retorno Acum Medio: %{y:.2f}%<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=results['Ganadores']['daily_ret'].index,
        y=results['Ganadores']['daily_ret'].mean(axis=1) * 100,  # Convertir a porcentaje
        mode='lines+markers',
        line=dict(color='green', width=3),
        marker=dict(size=6, symbol='diamond'),
        name='Retorno Acumulado Medio Ganadores',
        hovertemplate='Hora: %{x}<br>Retorno Acum Medio: %{y:.2f}%<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=results['Perdedores']['daily_ret'].index,
        y=results['Perdedores']['daily_ret'].mean(axis=1) * 100,  # Convertir a porcentaje
        mode='lines+markers',
        line=dict(color='red', width=3),
        marker=dict(size=6, symbol='diamond'),
        name='Retorno Acumulado Medio Perdedores',
        hovertemplate='Hora: %{x}<br>Retorno Acum Medio: %{y:.2f}%<extra></extra>'
    ))

    # Agregar scatter plots agrupados
    fig.add_trace(prepare_scatter_grouped(min_data, 'red', 'Mínimos', total_days))
    fig.add_trace(prepare_scatter_grouped(max_data, 'green', 'Máximos', total_days))
